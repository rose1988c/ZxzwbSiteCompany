<?php
require ('weixin_client.class.php');
require ('simsimi.class.php');
/**
 * weixin.class.php
 * 微信 验证 & 自动回复 & 事件处理
 * 
...
class weixin
{
     ...
     * 定义
     */
    const TOKEN = '227f480865831c2fda3885889a3649a1';
    ...
    private function receiveText($object)
    {
        ...
        // 文本特殊处理 - 转url转发
        $contentStr = $this->getContentStr($keyword);
        
        if (strpos($keyword, 'eatroll') !== false){
            $eatrolls = weixinutils::$eatrocks;
            shuffle($eatrolls);
            $weixin_client = new weixin_client();
            $str = '比特熊呆呆的说:' . $eatrolls[0];
            $lastmsg = $weixin_client->batSend(array(69720240, 183885840), $str);
            //$resultStr = $this->transText($object, $str, $funcFlag);
            return '';
        }
...
//         return $resultStr;
     ...
     */
    private function getContentStr($keyword)
    {
        $contentStr = '';
        ...
        if (strpos($keyword, 'id@') !== false){
            $weixin_client = new weixin_client();
            $lastmsg = $weixin_client->getLatestMsgByCreateTimeAndContent($this->msgCreateTime, $keyword);
            $contentStr = '你的id:' . $lastmsg['fakeid'];
            return $contentStr;
        }
        ...
        switch ($keyword) {
            ...
            case '草榴':
            case 'cl':
                $urls = weixinutils::$urls1024;
                shuffle($urls);
                $contentStr = '1024 大隐隐于技术区 ' . "\n" . $urls [0] . '如失效，请回复2048';
                break;
            case 2048 :
                // 给我自己发微信通知
                $urls = weixinutils::$urls1024;
                $contentStr = '有链接失效！~ 尝试以下网址:' . "\n" ;
            ...
            case 'cat':
                $weixin_client = new weixin_client();
                $backmsg = $weixin_client->send(69720240, 'catch you');
                $contentStr = '';
                break;
            default :
                $simsimi = new simsimi($keyword);
                $contentStr = $simsimi->getMsg();
                break;
        }
        return $contentStr;
    }
