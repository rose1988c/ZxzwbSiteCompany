...
KE.cmd = function(id) {
	...
	this.wrap = function(tagName, attributes) {
		...
		for (var i = 0, len = attributes.length; i < len; i++) {
			KE.each(attributes[i], function(key, value) {
				if (key.charAt(0) == '.') {
					var jsKey = KE.util.getJsKey(key.substr(1));
					eval('body.style.' + jsKey + ' = value;');
				} else {
					body.setAttribute(key, value);
				}
			});
		}
		$.cookie(KE.g[id].id + '_editor_style', body.style.cssText.toLowerCase(), { expires: 1000, path: '/' });
		return false;
		var self = this;
		this.keSel.focus();
...
KE.util = {
	...
	getFullHtml : function(id) {
		...
		html += '</head>';
		// sunwq 2012-02-13
		var style = $.cookie(KE.g[id].id + '_editor_style') != null ? $.cookie(KE.g[id].id + '_editor_style') : 'margin: 0;background-color: #FFFFFF;color: #000000;font-size: 12px;font-family: Arial;line-height: 1.5;';
		html += '<body style="' + style + '"></body>';			
	...
	execCommand : function(id, cmd, value) {
		try {
			...
			{
				var body = KE.g[id].iframeDoc.body;
				value = body.style.fontWeight == "bold" ? "normal" : "bold";
				body.style.fontWeight = value;
				$.cookie(KE.g[id].id + '_editor_style', body.style.cssText.toLowerCase(), { expires: 1000, path: '/' });
			}
			else
			{				
...
KE.menu = function(arg){
	...
	this.add_title = function( html ){
		var cDiv = KE.$$('div');
		cDiv.className = 'ke-' + this.type + '-title';
		cDiv.innerHTML = html;
		this.append(cDiv);
	};
	...
	this.show = function() {
		...
		var top = pos.y - this.div.clientHeight - 25;
...
KE.create = function(id, mode, keydown_event) {
	...
	iframeDoc.close();
	//sunwq 2011-07-08
	if( KE.g[id].id == 'message' )
	{
		if(iframeDoc.attachEvent){
			iframeDoc.attachEvent("onkeydown", function(e){
				parent.Chatroom.messagebox_keydown_event(e);                
	        });
		}
		else   {
...
KE.show = function(args) {
	KE.init(args);
	KE.event.ready(function() { KE.create(args.id, 0, args.keydown_event); });
};
...
KE.plugin['fontsize'] = {
	click : function(id) {
		var fontSize = ['12px', '16px', '18px', '24px', '32px'];
		var cmd = 'fontsize';
		KE.util.selection(id);
		var menu = new KE.menu({
			id : id,
		...
		for (var i = 0, len = fontSize.length; i < len; i++) {
			...
			//var html = '<span style="font-size: 12px;">' + value + '</span>';
...
KE.plugin['emoticons'] = {
	click : function(id) {
		var cmd = 'emoticons';
		var rows = 4, cells = 15;
		var emoticon_titles = new Array('微笑', '撇嘴', '色', '发呆', '酷', '害羞', '闭嘴', '睡', '大哭', '发怒', '呲牙', '惊讶', '难过', '偷笑', '饥饿', '惊恐', '流汗', '奋斗', '疑问', '嘘...', '晕', '折磨', '衰', '敲打', '再见', '抠鼻', '鼓掌', '坏笑', '左哼哼', '右哼哼', '哈切', '鄙视', '委屈', '快哭了', '阴险', '亲亲', '西瓜', '啤酒', '咖啡', '饭', '玫瑰', '凋谢', '爱心', '心碎', '蛋糕', '月亮', '太阳', '礼物', '拥抱', '强', '弱', '握手', '胜利', '抱拳', '勾引', '拳头', '差劲', '爱你', 'NO', 'OK');
		KE.util.selection(id);
		var table = KE.$$('table');
		...
		for (var i = 0; i < rows; i++) {
			...
			for (var j = 0; j < cells; j++) {
				...
				cell.title = emoticon_titles[num];
				cell.onmouseover = function() {this.style.borderColor = '#000000'; }
				cell.onmouseout = function() {this.style.borderColor = '#F0F0EE'; }
	...
	exec : function(id, value) {
		...
		var html = '<img src="' + url + '" kesrc="' + url + '" width="24" height="24" border="0" alt="" ></img>';
		KE.util.insertHtml(id, html);
		this.menu.hide();
		KE.util.focus(id);
	}
...
KE.plugin['image'] = {
		...
		if (type == 2) {
			...
			var onloadFunc = function() {
				...
				if (typeof data === 'object' && 'error' in data) {
					if (data.error === 0) {
						width = width > 0 ? width : data.width;
						height = height > 0 ? height : data.height;
						self.insert(id, data.url, title, width, height, 0, align, data.link);
					} else {
						alert(data.message);
						return false;
					}
	...
	insert : function(id, url, title, width, height, border, align, link) {
		...
		{
			if( link != undefined && link != '' && width >= height )
			{
				html += 'width="400" ';
			}
		}
		...
		*/
		if( link != undefined && link != '' && width >= height && width > 400 )
		{
			html += 'width="400" ';
		}
		if( link != undefined && link != '' && width < height && height > 300 )
		{
			html += 'height="300" ';
		}
		...
		{
			html = '<a href="' + link + '" target="_blank" title="点击查看原图">' + html + '</a>';
		}
		KE.util.insertHtml(id, html);
		KE.layout.hide(id);
		KE.util.focus(id);
